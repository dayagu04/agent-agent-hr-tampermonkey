        ...
        <Transition name="aah-fade" mode="out-in">
          ...
          <div v-if="activeTab === 'settings'" key="settings" class="aah-tab-pane aah-form-grid">
            ...
            <!-- 滑块：轨道越短越难精调（0~100 步进 5），给整行。
                 关闭匹配度时阈值不生效，滑块置灰防误导 -->
            <label
              class="aah-field aah-field-wide"
              :class="{ 'aah-disabled': !config.matchEnabled }"
            ...
            <!-- 投递间隔：单一配置 —— 投完一个岗位到投下一个的唯一等待来源 -->
            <label class="aah-field aah-field-wide">
              <span>投递间隔（秒）：投完一个岗位到下一个</span>
              <input type="number" min="0" max="600" v-model.number="config.applyIntervalSeconds" @change="persist" />
              <span class="aah-hint">
                内置 ±10% 随机抖动模拟真人；值越小投得越快，BOSS 风控风险越高，请自行评估
              </span>
            </label>
            ...
            <!-- 智能编排节奏：投多少个岗位后处理一次消息 / 每个关键词最多翻几页（0.4.11 起可配置） -->
            <label class="aah-field aah-field-wide">
              <span>编排节奏：每投 {{ config.chatCheckInterval }} 个岗位检查一次会话</span>
              <input type="number" min="1" max="100" v-model.number="config.chatCheckInterval" @change="persist" />
              <span class="aah-hint">
                智能编排的投递↔会话切换频率：越小检查越勤（更及时回 HR），越大投递越连贯（更快投完岗位）
              </span>
            </label>
            <label class="aah-field aah-field-wide">
              <span>编排节奏：每个关键词最多翻页数</span>
              <input type="number" min="1" max="100" v-model.number="config.maxPagesPerKeyword" @change="persist" />
              <span class="aah-hint">
                翻完自动切换下一个关键词，全部搜完则结束（默认 20 页 ≈ 300 个岗位）
              </span>
            </label>
            <label class="aah-field aah-field-wide">
              <div class="aah-row">

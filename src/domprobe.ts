...
//
// 为什么需要：BOSS/智联改版频繁，靠硬编码 class 猜选择器不可持续（已反复失败）。
// 本模块把页面真实结构 dump 出来，一次看清，并提供基于「文本/角色」的通用定位，
// 这类定位比 class 稳定得多（"发简历"这三个字比 .btn-resume-xxx 更不容易变）。
 ...
 * 找聊天消息面板。
 *
 * 教训：早期实现取「视口右半边面积最大的容器」，结果选中了插件自己的悬浮面板
 * （它就固定在右下角），导致消息一直读不到。
 *
 * 现在改用语义锚点：聊天区的可靠特征是「包含消息输入框」——
 * 从输入框往上找到那个既高又宽的祖先，即消息区所在容器。
 */
...
/** @deprecated 用 findChatPanel（旧名保留避免调用方报错） */
export const findRightPanel = findChatPanel

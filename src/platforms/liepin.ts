...
// 注：后端 web agent 未实现猎聘投递（仅插件支持），选择器需重点验证。
...
export class LiepinPlatform extends BasePlatform {
  ...
  async scanJobs(): Promise<JobCard[]> {
    // 猎聘搜索结果卡片（2026-07-13 验证通过）
    // 猎聘使用混淆class名（如 _40108yn42Q），需用属性选择器兜底
    ...
    for (const card of cards) {
      ...
      // 薪资：验证发现实际class为 _40108E8PWS（混淆），用属性选择器 + 关键词兜底
  ...
  async applyJob(card: JobCard): Promise<boolean> {
    // 猎聘投递通常需进详情页点「投递简历」。
    // TODO[真实验证]: 猎聘列表页一般无直接投递按钮，需进详情。
    //   且投递可能要求选择简历 + 登录态。这里实现详情页流程框架。
